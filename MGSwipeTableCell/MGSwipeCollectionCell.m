//
//  MGSwipeCollectionCell.m
//
//  Created by Daniel Morgan on 12/5/14.
//  Copyright (c) 2014 Imanol Fernandez Gorostizaga. All rights reserved.
//

#import "MGSwipeCollectionCell.h"

#pragma mark Input Overlay Helper Class
/** Used to capture table input while swipe buttons are visible*/
@interface MGSwipeCollectionInputOverlay : UIView
@property (nonatomic, weak) MGSwipeCollectionCell * currentCell;
@end

@implementation MGSwipeCollectionInputOverlay
...
@interface MGSwipeButtonView : UIView
@property (nonatomic, weak) MGSwipeCollectionCell * cell;
@end

@implementation MGSwipeButtonView
{
    NSArray * _buttons;
    UIView * _container;
    BOOL _fromLeft;
...
#pragma clang diagnostic pop
    ...
    if (_cell.delegate && [_cell.delegate respondsToSelector:@selector(swipeCollectionCell:tappedButtonAtIndex:direction:fromExpansion:)]) {
        NSInteger index = [_buttons indexOfObject:sender];
        if (!_fromLeft) {
            index = _buttons.count - index - 1; //right buttons are reversed
        }
        autoHide|= [_cell.delegate swipeCollectionCell:_cell tappedButtonAtIndex:index direction:_fromLeft ? MGSwipeDirectionLeftToRight : MGSwipeDirectionRightToLeft fromExpansion:fromExpansion];
    }
...
@implementation MGSwipeCollectionCell
{
    ...
    MGSwipeButtonView * _leftView;
    MGSwipeButtonView * _rightView;
    bool _allowSwipeRightToLeft;
    bool _allowSwipeLeftToRight;
    __weak MGSwipeButtonView * _activeExpansion;
    
    MGSwipeCollectionInputOverlay * _collectionInputOverlay;
    __weak UICollectionView * _cachedParentCollectionView;
...
{
    if (_leftButtons.count == 0 && _delegate && [_delegate respondsToSelector:@selector(swipeCollectionCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)]) {
        _leftButtons = [_delegate swipeCollectionCell:self swipeButtonsForDirection:MGSwipeDirectionLeftToRight swipeSettings:_leftSwipeSettings expansionSettings:_leftExpansion];
    }
    if (_rightButtons.count == 0 && _delegate && [_delegate respondsToSelector:@selector(swipeCollectionCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)]) {
        _rightButtons = [_delegate swipeCollectionCell:self swipeButtonsForDirection:MGSwipeDirectionRightToLeft swipeSettings:_rightSwipeSettings expansionSettings:_rightExpansion];
    }
}
...
-(void) createSwipeViewIfNeeded
{
    ...
    if (!_leftView && _leftButtons.count > 0) {
        _leftView = [[MGSwipeButtonView alloc] initWithButtons:_leftButtons direction:MGSwipeDirectionLeftToRight];
    ...
    if (!_rightView && _rightButtons.count > 0) {
        _rightView = [[MGSwipeButtonView alloc] initWithButtons:_rightButtons direction:MGSwipeDirectionRightToLeft];
...
- (void) showSwipeOverlayIfNeeded
{
    ...
    UICollectionView * collectionView = [self parentCollectionView];
    _collectionInputOverlay = [[MGSwipeCollectionInputOverlay alloc] initWithFrame:collectionView.bounds];
    _collectionInputOverlay.currentCell = self;
    [collectionView addSubview:_collectionInputOverlay];
...
-(void) hideSwipeOverlayIfNeeded
{
    ...
    [_collectionInputOverlay removeFromSuperview];
    _collectionInputOverlay = nil;
...
-(void) prepareForReuse
{
    ...
    [self cleanViews];
    BOOL cleanButtons = _delegate && [_delegate respondsToSelector:@selector(swipeCollectionCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)];
...
{
    if (_cachedParentCollectionView) {
        return _cachedParentCollectionView;
    }
    
    UIView * view = self.superview;
    while(view != nil) {
        if([view isKindOfClass:[UICollectionView class]]) {
            _cachedParentCollectionView = (UICollectionView*) view;
        }
        view = view.superview;
    }
    return _cachedParentCollectionView;
...
-(void) updateState: (MGSwipeState) newState;
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(swipeCollectionCell:didChangeSwipeState:gestureIsActive:)]) {
        [_delegate swipeCollectionCell:self didChangeSwipeState:_swipeState gestureIsActive: self.isSwipeGestureActive] ;
    }
}
...
- (void)setSwipeOffset:(CGFloat) newOffset;
{
    ...
    MGSwipeButtonView * activeButtons = sign < 0 ? _rightView : _leftView;
    ...
    MGSwipeButtonView* but[2] = {_leftView, _rightView};
...
-(void) panHandler: (UIPanGestureRecognizer *)gesture
{
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        MGSwipeButtonView * expansion = _activeExpansion;
        if (expansion) {
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if (gestureRecognizer == _panRecognizer) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(swipeCollectionCell:canSwipe:)]) {
            _allowSwipeLeftToRight = [_delegate swipeCollectionCell:self canSwipe:MGSwipeDirectionLeftToRight];
            _allowSwipeRightToLeft = [_delegate swipeCollectionCell:self canSwipe:MGSwipeDirectionRightToLeft];
        }
        else {
            [self fetchButtonsIfNeeded];
            _allowSwipeLeftToRight = _leftButtons.count > 0;
